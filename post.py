...
for f in sys.argv[1:]:
    ...
    newname = path+'/'+name.replace('wrfout', 'wrfout_sub')
    ...
    varlist = "LU_INDEX,ZNU,ZNW,ZS,DZS,NEST_POS,FNM,FNP,RDNW,RDN,DNW,DN,CFN,CFN1,RDX,RDY,RESM,CF1,CF2,CF3,TSLB,SEAICE,IVGTYP,ISLTYP,VEGFRA,AVG_FUEL_FRAC,ACGRDFLX,LAI,MAPFAC_M,F,E,TISO,MAX_MSTFX,MAX_MSTFY,OLR,ALBEDO,ALBBCK,ACHFX,ACLHF,SAVE_TOPO_FROM_REAL,SEED1,SEED2,XLAT_U,XLONG_U,XLAT_V,XLONG_V"
    levels = "-d bottom_top,0,9 -d bottom_top_stag,0,10"
    ...
    upper_name = path+'/'+name.replace('wrfout', 'wrfout_upr')
    ...
    if not os.path.exists(newname) or not os.path.exists(upper_name):
