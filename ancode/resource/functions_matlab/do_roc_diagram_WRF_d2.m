function [ hit_rate false_alarm_rate  ] = do_roc_diagram_WRF_d2( Namelist,analogs )
%DO_ROC_DIAGRAM Summary of this function goes here
%   Detailed explanation goes here

close all
    exp_name='Turbine_by_turbine_winter_1_mslp_wspd_wdir_rho_shear_domaine_2'
    ...
    load([file_path,filename]);;Namelist{4}.nwp_model_domain=2;Namelist{4}.nwp_model{1}='WRF'
    ...
    for j=analogs_lead_times
        ...
        % normalize all power data 
             ...
             first_time=1
             counter=0
             event_tresshold=0.50 % turbine producing less normalized power than event tresshold%
             treshold_counter=0
        ...
        for fixed_tresshold=fixed_tresshold_values
           ...
           false_alarm_rate=cat(2,ones(j,1),false_alarm_rate)
            ...
            title(['lead-',num2str(j),' nr verifing observation-',num2str(nr_obs)]);xlabel('False alarm rates');ylabel('Hit rates')
            save_dir=[Namelist{1}.stat_plot_dir,'\prob-plots\roc_curves']
            plot_filename=['\wrf_d2_roc_diagram_',nr_analogs,'_analogs_wrf_shear_turbine-lead-',num2str(j),'H']
                    if isdir(save_dir)
