function [ hit_rate false_alarm_rate  ] = do_roc_diagram_WRF_d3( Namelist,analogs )
...
close all
    ...
    ;Namelist{4}.nwp_model_domain=3;Namelist{4}.nwp_model{1}='WRF'
    ...
    for j=analogs_lead_times
        ...
        for fixed_tresshold=fixed_tresshold_values
           ...
           false_alarm_rate=cat(2,ones(j,1),false_alarm_rate)
            ...
            plot_filename=['\wrf_d3_roc_diagram_',nr_analogs,'_analogs_wrf_shear_turbine-lead-',num2str(j),'H']
                    if isdir(save_dir)
