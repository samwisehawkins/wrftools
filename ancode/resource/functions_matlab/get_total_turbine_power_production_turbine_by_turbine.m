function [ total_turbine_power_production ] = get_total_turbine_power_production_turbine_by_turbine( Namelist,turbine_data )
...
% make loop that runs throughthe 7 turbines and collects the observations on
% each hour 
...
% for all time between start date and end date, add 10 minutes 
% production if all turbiunes were running
dummy=1;
    total_turbine_power_production{2,1}=1
