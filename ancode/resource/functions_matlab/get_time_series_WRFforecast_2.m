function [data ]=get_time_series_WRFforecast_2(Namelist)
    first_time=1;
    Namelist{5}.currant_analog_period=Namelist{5}.start_analog_search_period
    num_now=datenum(Namelist{5}.currant_analog_period,'yyyy-mm-dd HH:MM')
    num_end=datenum(Namelist{5}.end_analog_search_period,'yyyy-mm-dd HH:MM')
while num_now<=num_end
        ddtg=datestr(num_now,'yyyy-mm-dd_HH')
  if strcmp(Namelist{4}.WRF_sst_fields,'41MH/')  
      domaine_group=['_','d0',num2str(Namelist{4}.nwp_model_domain),'_']
        file_name=strcat(Namelist{1}.forecast_data_dir,Namelist{2}.location{1},domaine_group,ddtg(1:length(ddtg)),'.dat')
  else
   domaine_group='_d0__'
   switch 1
    case Namelist{4}.nwp_model_domain==1
        file_name=strcat(Namelist{1}.forecast_data_dir,Namelist{2}.location{1},domaine_group,ddtg(2:length(ddtg)),'.dat')
    case Namelist{4}.nwp_model_domain==2
        file_name=strcat(Namelist{1}.forecast_data_dir,Namelist{2}.location{1},domaine_group,ddtg(2:length(ddtg)),'(1).dat')
    case Namelist{4}.nwp_model_domain==3
        file_name=strcat(Namelist{1}.forecast_data_dir,Namelist{2}.location{1},domaine_group,ddtg(2:length(ddtg)),'(2).dat')
    end    
  end 
      ...
      if first_time
            ...
            data{1,41}=vertcat(data{1,41},tempo{1,41});
             ...
             first_time=0;
              num_now=addtodate(num_now, 12, 'hour')
        else
            num_now=addtodate(num_now, 12, 'hour')
            % take next file pressent file does not exist
        end
end
    if (exist('data')==7)
        data=[]
