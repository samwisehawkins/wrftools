function [ succes ] = do_accuracy_concanated_turbines_10_analogs( Namelist )
...
close all
    ...
    for j=analogs_lead_times
        ...
        if not(isempty(obs))
                if Namelist{10}.normalize_errors
                    [ETA_rmse_10(j) ETA_bias_10(j) ETA_crmse_10(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
                    ETA_NMAE_10(j)=mean(abs(obs-model)/Namelist{10}.rated_capasity_kw);
                    ...
                    ETA_rmse_10(j) =mean(ETA_rmse_10(12:j-1));
                     ETA_bias_10(j) =mean(ETA_bias_10(12:j-1));
                     ETA_crmse_10(j) = mean(ETA_rmse_10(12:j-1));
                     ETA_NMAE_10(j)=mean(ETA_NMAE_10(12:j-1));
                     ETA_spearman_rank_correltion_10(j)=mean(ETA_spearman_rank_correltion_10(12:j-1))
    ...
    [m n]=size(turbine_time_series)    
        for i=1:n
           ...
           if not(isempty(good_idx{i}))
            if Namelist{10}.normalize_errors
                [WRF_d3_rmse_10(j) WRF_d3_bias_10(j) WRF_d3_crmse_10(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
                WRF_d3_NMAE_10(j)=mean(abs(obs-model)/Namelist{10}.rated_capasity_kw)
                WRF_d3_pearson_10(j)=corr(obs,model, 'Type', 'Pearson');
                WRF_d3_spearman_rank_correltion_10(j)=corr(obs,model, 'Type', 'Spearman');
            else
                [WRF_d3_rmse_10(j) WRF_d3_bias_10(j) WRF_d3_crmse_10(j)] = RMSEdecomp_all(obs, model);
                WRF_d3_NMAE_10(j)=mean(abs(obs-model))
                WRF_d3_pearson_10(j)=corr(obs,model, 'Type', 'Pearson');
                WRF_d3_spearman_rank_correltion_10(j)=corr(obs,model, 'Type', 'Spearman');
            end
           else % is empty 
                WRF_d3_rmse_10(j)=mean(WRF_d3_rmse_10(12:j-1)) 
                 WRF_d3_bias_10(j)=mean(WRF_d3_bias_10(12:j-1)) 
                 WRF_d3_crmse_10(j) = mean(WRF_d3_crmse_10(12:j-1))
                WRF_d3_NMAE_10(j)=mean(WRF_d3_NMAE_10(12:j-1))
                WRF_d3_pearson_10(j)=mean(WRF_d3_pearson_10(12:j-1))
                WRF_d3_spearman_rank_correltion_10(j)=mean(WRF_d3_spearman_rank_correltion_10(12:j-1))
           end % is empty    
    ...
    [m n]=size(turbine_time_series)    
        for i=1:n
           ...
           if not(isempty(obs))
            if Namelist{10}.normalize_errors
                [WRF_d2_rmse_10(j) WRF_d2_bias_10(j) WRF_d2_crmse_10(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
                WRF_d2_NMAE_10(j)=mean(abs(obs-model)/Namelist{10}.rated_capasity_kw)
                WRF_d2_pearson_10(j)=corr(obs,model, 'Type', 'Pearson');
                WRF_d2_spearman_rank_correltion_10(j)=corr(obs,model, 'Type', 'Spearman');
            else
                [WRF_d2_rmse_10(j) WRF_d2_bias_10(j) WRF_d2_crmse_10(j)] = RMSEdecomp_all(obs, model);
                WRF_d2_NMAE_10(j)=mean(abs(obs-model))
                WRF_d2_pearson_10(j)=corr(obs,model, 'Type', 'Pearson');
                WRF_d2_spearman_rank_correltion_10(j)=corr(obs,model, 'Type', 'Spearman');
            end
           else % isempty(obs)
                WRF_d2_rmse_10(j)=mean(WRF_d2_rmse_10(find(WRF_d2_rmse_10~=0))) 
                WRF_d2_bias_10(j)=mean(WRF_d2_bias_10(find(WRF_d2_bias_10~=0))) 
                WRF_d2_crmse_10(j) = mean(WRF_d2_crmse_10(find(WRF_d2_crmse_10~=0)))
                WRF_d2_NMAE_10(j)=mean(WRF_d2_NMAE_10(find(WRF_d2_NMAE_10~=0)))
                WRF_d2_pearson_10(j)=mean(WRF_d2_pearson_10(find(WRF_d2_pearson_10~=0)))
                WRF_d2_spearman_rank_correltion_10(j)=mean(WRF_d2_spearman_rank_correltion_10(find(WRF_d2_spearman_rank_correltion_10~=0)))
           end
    end % for lead times 
    ...
    [m n]=size(turbine_time_series)    
        for i=1:n
          ...
          model=vertcat(turbine_time_series(1,i).data{2,2}(good_idx{i}),model);
           ...
           if not(isempty(obs))
            if Namelist{10}.normalize_errors
               [WRF_d1_rmse_10(j) WRF_d1_bias_10(j) WRF_d1_crmse_10(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
                WRF_d1_NMAE_10(j)=mean(abs(obs-model)/Namelist{10}.rated_capasity_kw)
                WRF_d1_pearson_10(j)=corr(obs,model, 'Type', 'Pearson');
                WRF_d1_spearman_rank_correltion_10(j)=corr(obs,model, 'Type', 'Spearman');
            else
                [WRF_d1_rmse_10(j) WRF_d1_bias_10(j) WRF_d1_crmse_10(j)] = RMSEdecomp_all(obs, model);
                WRF_d1_NMAE_10(j)=mean(abs(obs-model))
                WRF_d1_pearson_10(j)=corr(obs,model, 'Type', 'Pearson');
                WRF_d1_spearman_rank_correltion_10(j)=corr(obs,model, 'Type', 'Spearman');
            end
           else
                WRF_d1_rmse_10(j)=mean(WRF_d1_rmse_10(find(WRF_d1_rmse_10~=0))) 
                WRF_d1_bias_10(j)=mean(WRF_d1_bias_10(find(WRF_d1_bias_10~=0))) 
                WRF_d1_crmse_10(j) = mean(WRF_d1_crmse_10(find(WRF_d1_crmse_10~=0)))
                WRF_d1_NMAE_10(j)=mean(WRF_d1_NMAE_10(find(WRF_d1_NMAE_10~=0)))
                WRF_d1_pearson_10(j)=mean(WRF_d1_pearson_10(find(WRF_d1_pearson_10~=0)))
                WRF_d1_spearman_rank_correltion_10(j)=mean(WRF_d1_spearman_rank_correltion_10(find(WRF_d1_spearman_rank_correltion_10~=0)))
           end % isempty
        for dummy=1:length(analogs_lead_times_d1)
            ...
            subplot (2,2,1)
               ...
               for i=1:n
                ...
                hold on;
                    ...
                    xlabel('Leadtime','fontsize',Namelist{7}.fontsize_sub_plot);set(gca,'fontsize',Namelist{7}.fontsize_sub_plot,'ylim',[0.12 0.25],'xtick',[1:1:length(analogs_lead_times_d3(lead_time_match_idx))], 'xtick',[1:1:length(analogs_lead_times_d3(lead_time_match_idx))], 'xticklabel',analogs_lead_times_d3(lead_time_match_idx)           );
            ...
            subplot (2,2,2)
               ...
               for i=1:n
                ...
                set(gca,'fontsize',Namelist{7}.fontsize_sub_plot,'ylim',[-0.08 0.04],'xtick',[1:1:length(analogs_lead_times_d3(lead_time_match_idx))], 'xticklabel',analogs_lead_times_d3(lead_time_match_idx)           );
            ...
            subplot (2,2,3)
               ...
               for i=1:n
                ...
                hold on;
                 ...
                 xlabel('Leadtime','fontsize',Namelist{7}.fontsize_sub_plot)
                 set(gca,'fontsize',Namelist{7}.fontsize_sub_plot,'ylim',[0.14 0.25],'xtick',[1:1:length(analogs_lead_times_d3(lead_time_match_idx))], 'xticklabel',analogs_lead_times_d3(lead_time_match_idx)           );
                legend({'ETA','WRF-d3','WRF-d2','WRF-d1'},'location','northeast')
           ...
           subplot (2,2,4)
               ...
               xlabel('Leadtime','fontsize',Namelist{7}.fontsize_sub_plot)
               set(gca,'fontsize',Namelist{7}.fontsize_sub_plot,'ylim',[0.35,0.85],'xtick',[1:1:length(analogs_lead_times_d3(lead_time_match_idx))], 'xticklabel',analogs_lead_times_d3(lead_time_match_idx)           );
               legend({'ETA','WRF-d3','WRF-d2','WRF-d1'},'location','northeast')
