function [ succes ] = do_dispersion_diagram_concanated_turbines_10_analogs( Namelist )
...
close all
    ...
    for j=analogs_lead_times
        ...
        if not(isempty(obs))
                if Namelist{10}.normalize_errors
                    ...
                    ETA_spread_10(j)=Namelist{1}.missing_value
        ...
        for i=1:n
           ...
           if not(isempty(good_idx{i}))
            if Namelist{10}.normalize_errors
                ...
                WRF_d3_spread_10(j)=sqrt(mean(model_ensembles_variance))
            else
                ...
                WRF_d3_spread_10(j)=Namelist{1}.missing_value
           end % is empty    
        ...
        for i=1:n
           ...
           if not(isempty(obs))
            if Namelist{10}.normalize_errors
                ...
                WRF_d2_spread_10(j)=sqrt(mean(model_ensembles_variance))
            else
                ...
                WRF_d2_spread_10(j)=Namelist{1}.missing_value
           end
    end % for lead times 
          ...
          model_ensembles_variance=vertcat(var(turbine_time_series(1,i).data{2,18}(good_idx{i},:)'/Namelist{10}.rated_capasity_kw)',model_ensembles_variance);
           ...
           if not(isempty(obs))
            if Namelist{10}.normalize_errors
               ...
               WRF_d1_spread_10(j)=sqrt(mean(model_ensembles_variance));
            else
                ...
                WRF_d1_spread_10(j)=Namelist{1}.missing_value
    ...
    mtx_spread(:,1)=ETA_spread_10(:,analogs_lead_times_d3(lead_time_match_idx));mtx_spread(:,:,2)=WRF_d3_spread_10(:,analogs_lead_times_d3(lead_time_match_idx));mtx_spread(:,:,3)=WRF_d2_spread_10(:,analogs_lead_times_d2);mtx_spread(:,:,4)=WRF_d1_spread_10(:,analogs_lead_times_d1)
    plot_width=2.5        
    subplot(2,2,1)
    ...
    [m n] =size(mtx_rmse);
        ...
        plot_filename=['\dispersion_diagran_10_analogs_wrf_shear_turbine']
                if isdir(save_dir)
