function [ succes ] = do_dispersion_diagram_concanated_turbines_05_analogs( Namelist )
...
close all
    ...
    load([file_path,'\turbine_time_series_for_nr_analogs_05'])
    ...
    for j=analogs_lead_times
        ...
        if not(isempty(obs))
                if Namelist{10}.normalize_errors
                    [ETA_rmse_05(j) ETA_bias_05(j) ETA_crmse_05(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
                    ETA_spread_05(j)=sqrt(mean(model_ensembles_variance))
                   
                else
                    [ETA_rmse_05(j) ETA_bias_05(j) ETA_crmse_05(j)] = RMSEdecomp_all(obs, model);
                   
                end
        else %isempty
                    ETA_rmse_05(j) =mean(ETA_rmse_05(12:j-1));
                    ETA_spread_05(j)=Namelist{1}.missing_value
    ...
    load([file_path,'\turbine_time_series_for_nr_analogs_5']);Namelist{4}.nwp_model_domain=3;Namelist{4}.nwp_model{1}='WRF'
    leadtime_vector=get_leadtime_vector(turbine_time_series,Namelist)'
    ...
    [m n]=size(turbine_time_series)    
        for i=1:n
           ...
           if not(isempty(good_idx{i}))
            if Namelist{10}.normalize_errors
                [WRF_d3_rmse_05(j) WRF_d3_bias_05(j) WRF_d3_crmse_05(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
                WRF_d3_spread_05(j)=sqrt(mean(model_ensembles_variance))
            else
                [WRF_d3_rmse_05(j) WRF_d3_bias_05(j) WRF_d3_crmse_05(j)] = RMSEdecomp_all(obs, model);
                
            end
           else % is empty 
                WRF_d3_rmse_05(j)=mean(WRF_d3_rmse_05(12:j-1)) 
                WRF_d3_spread_05(j)=Namelist{1}.missing_value
           end % is empty    
    ...
    [m n]=size(turbine_time_series)    
        for i=1:n
           ...
           if not(isempty(obs))
            if Namelist{10}.normalize_errors
                [WRF_d2_rmse_05(j) WRF_d2_bias_05(j) WRF_d2_crmse_05(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
                WRF_d2_spread_05(j)=sqrt(mean(model_ensembles_variance))
            else
                [WRF_d2_rmse_05(j) WRF_d2_bias_05(j) WRF_d2_crmse_05(j)] = RMSEdecomp_all(obs, model);
                
            end
           else % isempty(obs)
                WRF_d2_rmse_05(j)=mean(WRF_d2_rmse_05(find(WRF_d2_rmse_05~=0)))                
                WRF_d2_spread_05(j)=Namelist{1}.missing_value
           end
    end % for lead times 
    ...
    [m n]=size(turbine_time_series)    
        for i=1:n
          ...
          model_ensembles_variance=vertcat(var(turbine_time_series(1,i).data{2,18}(good_idx{i},:)'/Namelist{10}.rated_capasity_kw)',model_ensembles_variance);
           ...
           if not(isempty(obs))
            if Namelist{10}.normalize_errors
               [WRF_d1_rmse_05(j) WRF_d1_bias_05(j) WRF_d1_crmse_05(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
               WRF_d1_spread_05(j)=sqrt(mean(model_ensembles_variance));
            else
                [WRF_d1_rmse_05(j) WRF_d1_bias_05(j) WRF_d1_crmse_05(j)] = RMSEdecomp_all(obs, model);
               
            end
           else
                WRF_d1_rmse_05(j)=mean(WRF_d1_rmse_05(find(WRF_d1_rmse_05~=0))) 
                WRF_d1_spread_05(j)=Namelist{1}.missing_value
    ...
    mtx_spread(:,1)=ETA_spread_05(:,analogs_lead_times_d3(lead_time_match_idx));mtx_spread(:,:,2)=WRF_d3_spread_05(:,analogs_lead_times_d3(lead_time_match_idx));mtx_spread(:,:,3)=WRF_d2_spread_05(:,analogs_lead_times_d2);mtx_spread(:,:,4)=WRF_d1_spread_05(:,analogs_lead_times_d1)
    plot_width=2.5        
    subplot(2,2,1)
    ...
    [m n] =size(mtx_rmse);
             for i=2 % for number of models consideres 
                ...
                grid on;ylabel('RMSE','fontsize',Namelist{7}.fontsize_sub_plot);
                xlabel('Leadtime','fontsize',Namelist{7}.fontsize_sub_plot);set(gca,'fontsize',Namelist{7}.fontsize_sub_plot,'ylim',[0.10 0.30],'xticklabel',analogs_lead_times_d3(lead_time_match_idx([1 2 3 4 6 7 8 9])));
                legend({'WRF 2km-rmse','WRF 2km-spread'})
    ...
    [m n] =size(mtx_rmse);
             for i=3 % for number of models consideres 
                ...
                grid on;ylabel('RMSE','fontsize',Namelist{7}.fontsize_sub_plot);
                xlabel('Leadtime','fontsize',Namelist{7}.fontsize_sub_plot);set(gca,'fontsize',Namelist{7}.fontsize_sub_plot,'ylim',[0.10 0.30],'xticklabel',analogs_lead_times_d3(lead_time_match_idx([1 2 3 4 6 7 8 9])));
                legend({'WRF 6km rmse','WRF 6km spread'})
    ...
    [m n] =size(mtx_rmse);
             for i=4 % for number of models consideres 
                ...
                grid on;ylabel('RMSE','fontsize',Namelist{7}.fontsize_sub_plot);
                xlabel('Leadtime','fontsize',Namelist{7}.fontsize_sub_plot);set(gca,'fontsize',Namelist{7}.fontsize_sub_plot,'ylim',[0.10 0.30],'xticklabel',analogs_lead_times_d3(lead_time_match_idx([1 2 3 4 6 7 8 9])));
                legend({'WRF 18 km RMSE','WRF 18 km spread'})

        maximize
        save_dir=[Namelist{1}.stat_plot_dir,'\prob-plots']
        plot_filename=['\dispersion_diagran_05_analogs_wrf_shear_turbine']
                if isdir(save_dir)
