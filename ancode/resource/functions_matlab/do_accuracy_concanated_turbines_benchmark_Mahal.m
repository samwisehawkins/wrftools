function [ succes ] = do_accuracy_concanated_turbines_benchmark_Mahal(Namelist)
...
close all
    ...
    exp_name_wrf='Mahal_exp_Weighting_1_2010-10-31 12_00_2012-03-02 00_00_domaine_3'
    file_path=['C:\Users\jnini\MATLAB\work\AnEn\data\workspace\WRF\out\experiments\',exp_name_wrf]
    load([file_path,'\turbine_time_series_for_nr_analogs_20'])
    ...
    [m n]=size(turbine_time_series)    
        for i=1:n
           ...
           if not(isempty(obs))
            if Namelist{10}.normalize_errors
                [WRF_d3_mahal_rmse_20(j) WRF_d3_mahal_bias_20(j) WRF_d3_mahal_crmse_20(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
                WRF_d3_mahal_NMAE_20(j)=mean(abs(obs-model)/Namelist{10}.rated_capasity_kw)
                WRF_d3_mahal_pearson_20(j)=corr(obs,model, 'Type', 'Pearson');
                WRF_d3_mahal_spearman_rank_correltion_20(j)=corr(obs,model, 'Type', 'Spearman');
            else
                [WRF_d3_mahal_rmse_20(j) WRF_d3_mahal_bias_20(j) WRF_d3_mahal_crmse_20(j)] = RMSEdecomp_all(obs, model);
                WRF_d3_mahal_NMAE_20(j)=mean(abs(obs-model))
                WRF_d3_mahal_pearson_20(j)=corr(obs,model, 'Type', 'Pearson');
                WRF_d3_mahal_spearman_rank_correltion_20(j)=corr(obs,model, 'Type', 'Spearman');
            end
           else % isempty(obs)
                WRF_d3_mahal_rmse_20(j)=mean(WRF_d3_mahal_rmse_20(find(WRF_d3_mahal_rmse_20~=0))) 
                WRF_d3_mahal_bias_20(j)=mean(WRF_d3_mahal_bias_20(find(WRF_d3_mahal_bias_20~=0))) 
                WRF_d3_mahal_crmse_20(j) = mean(WRF_d3_mahal_crmse_20(find(WRF_d3_mahal_crmse_20~=0)))
                WRF_d3_mahal_NMAE_20(j)=mean(WRF_d3_mahal_NMAE_20(find(WRF_d3_mahal_NMAE_20~=0)))
                WRF_d3_mahal_pearson_20(j)=mean(WRF_d3_mahal_pearson_20(find(WRF_d3_mahal_pearson_20~=0)))
                WRF_d3_mahal_spearman_rank_correltion_20(j)=mean(WRF_d3_mahal_spearman_rank_correltion_20(find(WRF_d3_mahal_spearman_rank_correltion_20~=0)))
           end
    end % for lead times 
     ...
     for dummy=1:length(analogs_lead_times_d1)
             ...
             mtx_nmae(:,1)=ETA_NMAE_20(:,analogs_lead_times_d3(lead_time_match_idx));mtx_nmae(:,:,2)=WRF_d3_NMAE_20(:,analogs_lead_times_d3(lead_time_match_idx));mtx_nmae(:,:,3)=WRF_d3_mahal_NMAE_20(:,analogs_lead_times_d2);
             mtx_bias(:,1)=ETA_bias_20(:,analogs_lead_times_d3(lead_time_match_idx));mtx_bias(:,:,2)=WRF_d3_bias_20(:,analogs_lead_times_d3(lead_time_match_idx));mtx_bias(:,:,3)=WRF_d3_mahal_bias_20(:,analogs_lead_times_d2);
             mtx_rmse(:,1)=ETA_rmse_20(:,analogs_lead_times_d3(lead_time_match_idx));mtx_rmse(:,:,2)=WRF_d3_rmse_20(:,analogs_lead_times_d3(lead_time_match_idx));mtx_rmse(:,:,3)=WRF_d3_mahal_rmse_20(:,analogs_lead_times_d2);
             mtx_crmse(:,1)=      ETA_crmse_20(:,analogs_lead_times_d3(lead_time_match_idx));mtx_crmse(:,:,2)=WRF_d3_crmse_20(:,analogs_lead_times_d3(lead_time_match_idx));mtx_crmse(:,:,3)=WRF_d3_mahal_crmse_20(:,analogs_lead_times_d2);
             mtx_pearson(:,1)=ETA_pearson_20(:,analogs_lead_times_d3(lead_time_match_idx));mtx_pearson(:,:,2)=WRF_d3_pearson_20(:,analogs_lead_times_d3(lead_time_match_idx));mtx_pearson(:,:,3)=WRF_d3_mahal_pearson_20(:,analogs_lead_times_d2);
             mtx_spearman_rank_correltion(:,1)=ETA_spearman_rank_correltion_20(:,analogs_lead_times_d3(lead_time_match_idx));mtx_spearman_rank_correltion(:,:,2)=WRF_d3_spearman_rank_correltion_20(:,analogs_lead_times_d3(lead_time_match_idx));mtx_spearman_rank_correltion(:,:,3)=WRF_d3_mahal_spearman_rank_correltion_20(:,analogs_lead_times_d2);

            subplot (2,6,1:4)
               [m n] =size(mtx_nmae);
               for i=1:n
                plotmtx=mtx_nmae(:,i);p=plot(plotmtx,['-' Namelist{5}.markers(i) Namelist{5}.color{7+i}],'linewidth',2.5);
                hold on;
               end
                grid on;ylabel('NMAE','fontsize',Namelist{7}.fontsize_sub_plot);
                xlabel('Leadtime','fontsize',Namelist{7}.fontsize_sub_plot);set(gca,'fontsize',Namelist{7}.fontsize_sub_plot,'ylim',[0.12 0.20],'xtick',[1:1:length(analogs_lead_times_d3(lead_time_match_idx))],'xticklabel',analogs_lead_times_d3(lead_time_match_idx));
                legend({'ETA','WRF-d3','WRF-d3-Mahal'},'location','northeast')

            subplot (2,6,7:10)
               [m n] =size(mtx_nmae);
               for i=1:n
                plotmtx=mtx_spearman_rank_correltion(:,i);p=plot(plotmtx,['-' Namelist{5}.markers(i) Namelist{5}.color{7+i}],'linewidth',2.5);
                hold on;
               end
                grid on;ylabel('Correlation','fontsize',Namelist{7}.fontsize_sub_plot);
                xlabel('Leadtime','fontsize',Namelist{7}.fontsize_sub_plot);set(gca,'fontsize',Namelist{7}.fontsize_sub_plot,'ylim',[0.45,0.8],'xtick',[1:1:length(analogs_lead_times_d3(lead_time_match_idx))],'xticklabel',analogs_lead_times_d3(lead_time_match_idx));
                legend({'ETA','WRF-d3','WRF-d3-Mahal'},'location','northeast')
    
        maximize
        save_dir=[Namelist{1}.stat_plot_dir,'\accuracy_nwp_turbine_concanated_on_leadtimes']
        plot_filename=['\stats_on_nr_20_analogs_nwp_dom_3_mahal']
                if isdir(save_dir)
