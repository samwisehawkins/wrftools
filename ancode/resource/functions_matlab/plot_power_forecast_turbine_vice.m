...
for turbine_counter=1:n
 ...
 plot(time_serie_power_forecast(1,turbine_counter).data{2,2}(length(time_serie_power_forecast(1,turbine_counter).data{2,2})-((length(Namelist{5}.Analog.lead_times)-1)/Namelist{5}.Analog.lead_delta):length(time_serie_power_forecast(1,turbine_counter).data{2,2}))/Namelist{10}.rated_capasity_kw,'--r*','LineWidth',2,...
                'MarkerEdgeColor','r',...
                'MarkerFaceColor','r',...
                'MarkerSize',10)
hold on 
if Namelist{10}.do_regression
plot(time_serie_power_forecast(1,turbine_counter).data{2,19}(length(time_serie_power_forecast(1,turbine_counter).data{2,2})-((length(Namelist{5}.Analog.lead_times)-1)/Namelist{5}.Analog.lead_delta):length(time_serie_power_forecast(1,turbine_counter).data{2,2}))/Namelist{10}.rated_capasity_kw,':black*','LineWidth',1,...
                'MarkerEdgeColor','g',...
                'MarkerFaceColor','g',...
                'MarkerSize',10)
end
    ...
    %legend({'Deterministic forecast','obs'})
        ...
        % plot the bands 
        x=[1:length(time_serie_power_forecast(1,turbine_counter).data{2,7}(length(time_serie_power_forecast(1,turbine_counter).data{2,2})-((length(Namelist{5}.Analog.lead_times)-1)/Namelist{5}.Analog.lead_delta):length(time_serie_power_forecast(1,turbine_counter).data{2,2})))]
        for i=1:Namelist{5}.number_ci_plots % upper bands first 
            h(i)=ciplot(time_serie_power_forecast(1,turbine_counter).data{2,7+i-1}(length(time_serie_power_forecast(1,turbine_counter).data{2,2})-((length(Namelist{5}.Analog.lead_times)-1)/Namelist{5}.Analog.lead_delta):length(time_serie_power_forecast(1,turbine_counter).data{2,2}))/Namelist{10}.rated_capasity_kw,time_serie_power_forecast(1,turbine_counter).data{2,7+i}(length(time_serie_power_forecast(1,turbine_counter).data{2,2})-((length(Namelist{5}.Analog.lead_times)-1)/Namelist{5}.Analog.lead_delta):length(time_serie_power_forecast(1,turbine_counter).data{2,2}))/Namelist{10}.rated_capasity_kw,x,[0.2989 + 0.5870  + 0.1140],0.75/i)
            hold on
...
%            legend({,'Deterministic forecast','obs','50-60% percentile','40-50% percentile','60-70% percentile','30-40% percentile','70-80% percentile','20-30% percentile'})
            ...
            set(gca,'xtick',[1:6:nr_data_points],'xticklabels',date_labels([1:6:((length(Namelist{5}.Analog.lead_times)-1)/Namelist{5}.Analog.lead_delta)],:),'fontsize',Namelist{7}.fontsize_sub_plot-1)
...
figure;label_counter=0;
            for turbine_counter_2=1:n
                plot(time_serie_power_forecast(1,turbine_counter_2).data{2,2}(length(time_serie_power_forecast(1,turbine_counter_2).data{2,2})-((length(Namelist{5}.Analog.lead_times)-1)/Namelist{5}.Analog.lead_delta):length(time_serie_power_forecast(1,turbine_counter_2).data{2,2}))/Namelist{10}.rated_capasity_kw,strcat('-r',Namelist{5}.markers(turbine_counter_2)),'LineWidth',1,...
                'MarkerEdgeColor','r',...
                'MarkerFaceColor','r',...
                'MarkerSize',10);
            ...
            label_str{label_counter}=['P-obs:',num2str(power_obs_vector(1,turbine_counter_2).turbineid(1))];
              ...
              if strcmp(computer('arch'),'win32') 
               ...
               set(gca,'xlim',[1 nr_data_points],'ylim',[0 1]); grid on; colormap('gray')
                ...
                date_labels=power_obs_vector(1,turbine_counter).valid_date(:,11:16)
                set(gca,'xtick',[1:4:nr_data_points],'xticklabels',date_labels([1:4:((length(Namelist{5}.Analog.lead_times)-1)/Namelist{5}.Analog.lead_delta)],:),'fontsize',Namelist{7}.fontsize_sub_plot)
