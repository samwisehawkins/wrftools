function [ succes ] = do_dispersion_diagram_concanated_turbines_15_analogs( Namelist )
...
close all
    ...
    for j=analogs_lead_times
        ...
        if not(isempty(obs))
                if Namelist{10}.normalize_errors
                    [ETA_rmse_15(j) ETA_bias_15(j) ETA_crmse_15(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
                    ETA_spread_15(j)=sqrt(mean(model_ensembles_variance))
                   
                else
                    [ETA_rmse_15(j) ETA_bias_15(j) ETA_crmse_15(j)] = RMSEdecomp_all(obs, model);
                   
                end
        else %isempty
                    ETA_rmse_15(j) =mean(ETA_rmse_15(12:j-1));
                    ETA_spread_15(j)=Namelist{1}.missing_value
    ...
    load([file_path,'\turbine_time_series_for_nr_analogs_15']);Namelist{4}.nwp_model_domain=3;Namelist{4}.nwp_model{1}='WRF'
    leadtime_vector=get_leadtime_vector(turbine_time_series,Namelist)'
    ...
    [m n]=size(turbine_time_series)    
        for i=1:n
           ...
           if not(isempty(good_idx{i}))
            if Namelist{10}.normalize_errors
                [WRF_d3_rmse_15(j) WRF_d3_bias_15(j) WRF_d3_crmse_15(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
                WRF_d3_spread_15(j)=sqrt(mean(model_ensembles_variance))
            else
                [WRF_d3_rmse_15(j) WRF_d3_bias_15(j) WRF_d3_crmse_15(j)] = RMSEdecomp_all(obs, model);
                
            end
           else % is empty 
                WRF_d3_rmse_15(j)=mean(WRF_d3_rmse_15(12:j-1)) 
                WRF_d3_spread_15(j)=Namelist{1}.missing_value
           end % is empty    
    ...
    [m n]=size(turbine_time_series)    
        for i=1:n
           ...
           if not(isempty(obs))
            if Namelist{10}.normalize_errors
                [WRF_d2_rmse_15(j) WRF_d2_bias_15(j) WRF_d2_crmse_15(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
                WRF_d2_spread_15(j)=sqrt(mean(model_ensembles_variance))
            else
                [WRF_d2_rmse_15(j) WRF_d2_bias_15(j) WRF_d2_crmse_15(j)] = RMSEdecomp_all(obs, model);
                
            end
           else % isempty(obs)
                WRF_d2_rmse_15(j)=mean(WRF_d2_rmse_15(find(WRF_d2_rmse_15~=0)))                
                WRF_d2_spread_15(j)=Namelist{1}.missing_value
           end
    end % for lead times 
    ...
    [m n]=size(turbine_time_series)    
        for i=1:n
          ...
          model_ensembles_variance=vertcat(var(turbine_time_series(1,i).data{2,18}(good_idx{i},:)'/Namelist{10}.rated_capasity_kw)',model_ensembles_variance);
           ...
           if not(isempty(obs))
            if Namelist{10}.normalize_errors
               [WRF_d1_rmse_15(j) WRF_d1_bias_15(j) WRF_d1_crmse_15(j)] = RMSEdecomp_all(obs/Namelist{10}.rated_capasity_kw, model/Namelist{10}.rated_capasity_kw);
               WRF_d1_spread_15(j)=sqrt(mean(model_ensembles_variance));
            else
                [WRF_d1_rmse_15(j) WRF_d1_bias_15(j) WRF_d1_crmse_15(j)] = RMSEdecomp_all(obs, model);
               
            end
           else
                WRF_d1_rmse_15(j)=mean(WRF_d1_rmse_15(find(WRF_d1_rmse_15~=0))) 
                WRF_d1_spread_15(j)=Namelist{1}.missing_value
    ...
    mtx_spread(:,1)=ETA_spread_15(:,analogs_lead_times_d3(lead_time_match_idx));mtx_spread(:,:,2)=WRF_d3_spread_15(:,analogs_lead_times_d3(lead_time_match_idx));mtx_spread(:,:,3)=WRF_d2_spread_15(:,analogs_lead_times_d2);mtx_spread(:,:,4)=WRF_d1_spread_15(:,analogs_lead_times_d1)
    plot_width=2.5        
    subplot(2,2,1)
    ...
    [m n] =size(mtx_rmse);
        ...
        plot_filename=['\dispersion_diagran_15_analogs_wrf_shear_turbine']
                if isdir(save_dir)
